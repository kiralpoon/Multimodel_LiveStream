...
def setup_logger():
    logger = logging.getLogger("fish_audio_api_logger")
...
ap.add_argument('--port', type=int , default=32551)  # Different port to avoid conflicts
ap.add_argument('--model', type=str , default="openbmb/MiniCPM-o-2_6", help="huggingface model name or local path")
args = ap.parse_args()


class FishAudioStreamManager:
    def __init__(self):
        self.uid = None
        ...
        self.timing_stats = {
            'asr_time': 0.0,
            'llm_time': 0.0, 
            'fish_audio_time': 0.0,  # Changed from tts_time to fish_audio_time
            'total_time': 0.0
        }
        ...
        # FISH AUDIO INTEGRATION: Remove local TTS initialization
        # self.minicpmo_model.init_tts()
        # self.minicpmo_model.tts.float()
        
        # FISH AUDIO INTEGRATION: Initialize Fish Audio client
        try:
            self.fish_audio_client = FishAudioClient()
            logger.info("Fish Audio client initialized successfully")
        except Exception as e:
            logger.error(f"Failed to initialize Fish Audio client: {e}")
            logger.error("Please ensure you have:")
            logger.error("1. Created a .env file in the web_server directory")
            logger.error("2. Added your Fish Audio API key: FISH_AUDIO_API_KEY=your_key_here")
            logger.error("3. The .env file is in the correct location: web_demos/minicpm-o_2.6/web_server/.env")
            raise
        
        # FISH AUDIO INTEGRATION: Fish Audio configuration
        self.fish_audio_config = {
            'model': 'speech-1.5',  # Default model
            'format': 'wav',        # Default format
            'sample_rate': 44100,   # Default sample rate
            'voice_id': None        # No custom voice by default
        }
        
        self.minicpmo_model.to(self.device).eval()

        # Fix paths to be dynamic based on where the server is run from
        import pathlib
        current_dir = pathlib.Path(__file__).parent
        
        # The server file is in web_demos/minicpm-o_2.6/, so go up to the main project root
        # where the assets directory is located
        project_root = current_dir.parent.parent  # Go up from web_demos/minicpm-o_2.6/ to web_demos/ to project root
        
        self.ref_path_video_default = str(project_root / "assets" / "ref_audios" / "video_default.wav")
        self.ref_path_default = str(project_root / "assets" / "ref_audios" / "default.wav")
        self.ref_path_female = str(project_root / "assets" / "ref_audios" / "female_example.wav")
        self.ref_path_male = str(project_root / "assets" / "ref_audios" / "male_example.wav")
    ...
    def sys_prompt_init(self, msg_type):
        ...
        if msg_type <= 1: #audio
            ...
            if not os.path.exists(ref_path):
                ...
                logger.error("Please ensure you're running the server from the project root directory")
                logger.error("Expected path: /mnt/d/Projects/MiniCPM-o/web_demos/minicpm-o_2.6/")
        ...
        try:
            with torch.no_grad():
                ...
                with torch.inference_mode():
                    ...
                    fish_audio_start_time = None
                    
                    try:
                        # FISH AUDIO INTEGRATION: Collect complete text first
                        complete_text = ""
                        
                        for r in self.minicpmo_model.streaming_generate(
                            session_id=str(self.session_id),
                            tokenizer=self.minicpmo_tokenizer,
                            generate_audio=False,  # Don't generate audio locally
                        ...
                        # Clean up the complete text - remove TTS tokens and other artifacts
                        complete_text = self.clean_text_for_tts(complete_text)
                            
                        # FISH AUDIO INTEGRATION: Send complete text to Fish Audio API
                        fish_audio_start_time = time.time()
                        
                        try:
                            audio_response = await self.fish_audio_client.tts(
                                text=complete_text,
                                model=self.fish_audio_config['model'],
                                voice_id=self.fish_audio_config['voice_id'],
                                format=self.fish_audio_config['format'],
                                sample_rate=self.fish_audio_config['sample_rate']
                            )
                            
                            # Calculate Fish Audio API time
                            fish_audio_end_time = time.time()
                            self.timing_stats['fish_audio_time'] = round((fish_audio_end_time - fish_audio_start_time) * 1000, 1)
                            
                            logger.info(f"Fish Audio TTS successful: {len(audio_response)} bytes, "
                                       f"text length: {len(complete_text)}, "
                                       f"time: {self.timing_stats['fish_audio_time']}ms")
                            
                            # Save audio response for logging
                            ...
                            yield base64.b64encode(audio_response).decode('utf-8'), complete_text
                            
                        except Exception as e:
                            logger.error(f"Fish Audio TTS failed: {str(e)}")
                            # Fallback: return text only
                            yield None, complete_text
                            
                    except Exception as e:
                        logger.error(f"Error happened during generation: {str(e)}")
                    
                    # Send timing stats with final message
                    timing_msg = f"⏱️ ASR: {self.timing_stats['asr_time']}ms | LLM: {self.timing_stats['llm_time']}ms | Fish Audio: {self.timing_stats['fish_audio_time']}ms | Total: {round((time.time() - total_start_time) * 1000, 1)}ms"
    ...
    def clean_text_for_tts(self, text):
        """Clean text by removing TTS tokens and other artifacts"""
        if not text:
            return text
        
        # Remove TTS end-of-sequence tokens
        text = text.replace('<|tts_eos|>', '')
        text = text.replace('<|TTS_EOS|>', '')
        
        # Remove other potential TTS tokens
        text = text.replace('<|tts_start|>', '')
        text = text.replace('<|TTS_START|>', '')
        
        # Remove any remaining angle bracket tokens that might be TTS-related
        import re
        text = re.sub(r'<\|[^>]*\|>', '', text)
        ...
        logger.info(f"Cleaned text for TTS: '{text}'")
        return text
...
stream_manager = FishAudioStreamManager()


@app.on_event("startup")
async def startup_event():
    logger.info("Starting Fish Audio application and activity checker")
    asyncio.create_task(stream_manager.check_activity())

@app.on_event("shutdown")
async def shutdown_event():
    logger.info("Shutting down Fish Audio application")
        ...
        for message in messages:
            ...
            for content in message.get("content", []):
                if content["type"] == "input_audio":
                    ...
                    # FISH AUDIO INTEGRATION: Update Fish Audio configuration
                    if "fish_audio" in content["options"]:
                        fish_audio_opts = content["options"]["fish_audio"]
                        if "model" in fish_audio_opts:
                            stream_manager.fish_audio_config['model'] = fish_audio_opts["model"]
                        if "format" in fish_audio_opts:
                            stream_manager.fish_audio_config['format'] = fish_audio_opts["format"]
                        if "sample_rate" in fish_audio_opts:
                            stream_manager.fish_audio_config['sample_rate'] = fish_audio_opts["sample_rate"]
                        if "voice_id" in fish_audio_opts:
                            stream_manager.fish_audio_config['voice_id'] = fish_audio_opts["voice_id"]
                else:
                    ctype = content["type"]
...
async def health_check():
    ...
    return {"status": "healthy", "service": "fish_audio_model_server"}
